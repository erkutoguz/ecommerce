...
import dev.erkut.paymentservice.outbox.domain.exception.InvalidOutboxMessageException;
    ...
    public CommonErrorHandler kafkaErrorHandler(
        ...
        DeadLetterPublishingRecoverer recoverer =
                new DeadLetterPublishingRecoverer(
                        kafkaTemplate,
                        (record, exception) -> {
                            if (record.topic().equals(topics.paymentCommands())) {
                                return new TopicPartition(
                                        topics.paymentCommandsDlt(),
                                        record.partition()
                                );
                            }
